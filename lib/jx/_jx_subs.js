...
process.keepAlive = function (ms) {
  if (process.__reset)
    return;
  incWaitCounter();
  tw.sendToAll(-1, JSON.stringify({threadId: process.threadId, params: null, wait: 1}), process.threadId);
  if (ms) {
    setTimeout(function () {
      process.release();
    }, ms, 1);
...
process.release = function (customCounter, returnCb) {
  if (getWaitCounter() <= 0)
    return;
  if (!process.__reset)
    customCounter = null;
  ...
  if (!restarted && !process.__reset) {
    setTimeout(function () {
      ...
      tw.sendToAll(-1, JSON.stringify({threadId: process.threadId, params: null, wait: -1 * num, returnValue: returnCb}), process.threadId);
...
var strGlobals = "/**/__dirname = '" + path.dirname(resolved) + "';__filename = '" + resolved + "';process.mainModule.filename='" + resolved + "';";
if (isWindows) {
...
var func = function (scr) {
  ...
  if (ind < 0) {
    ex_data.setMap(process.threadId, "*" + name, strGlobals + "var ___method=" + scr[1] + ";"
    + "exports.call = function(___cbid, ___param){"
    + "var ___x=___method(___param);"
    + 'if(!___x || ___cbid==-1){return JSON.stringify({"_id":___cbid, dummy:true})};'
    ...
    ex_data.setMap(process.threadId, "*" + name, embedFunc(strGlobals, strd,
      ';var _____cl=false;var continueLogic=function(){}, ____back=[];var ___method=' + strl
      ...
      + 'if(___x==undefined || ___cbid==-1){return JSON.stringify({"_id":___cbid, dummy:true})};'
      + 'return JSON.stringify({"_id":___cbid, "o":___x});'
      + '}catch(e){console.log("error during task execution:",e);throw "task execution: " + e;}'
      + '};', sub2));
  }
};
...
process.on('$$restart', function (err) {
  try {
    ...
    if (!process.emit('restart', process.unloadThread)) {
      ...
      if (err) {
        console.error("restarting thread " + process.threadId + " due to error", err.toString());
      }
      setTimeout(function () {
