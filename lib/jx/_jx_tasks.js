  ...
  for (var o in arr) {
    ...
    if (obj.threadId || obj.threadId === 0 || obj.tid || obj.tid === 0) {
      if (obj.resetMe) {
        ...
        var tot = 100;
        if (obj.threadId > 0) tot = 175 * (obj.threadId);
    ...
    if( obj.returnValue ){
      obj._id = obj.returnValue[0];
      obj.o = obj.returnValue[1];
    }else{
      cbb++;
    }
    ...
    if (cb && obj.o !== '_deferCb') {
      if (obj.dummy) {
        cb();
      } else if (!objects[obj._id]) {
        cb(obj.o);
      } else {
        cb(objects[obj._id], obj.o);
...
var getMethod = function (method) {
  ...
  if (method.define || method.logic) {
    if (!method.define) {
      throw new Error("undefined ::logic for the task");
    }
    if (!method.logic) {
      method.logic = function () {
      };
    }
    if (!isFunction(method.define) && !method.define.substr) {
      throw new TypeError(
...
exports._runOnce = function (method, param, doNotRemember, skip_thread_creation) {
  ...
  var err = uw.addTask(taskId, mt, param, -2, doNotRemember, skip_thread_creation);
  if (err > 0) {
    throw new Error("Thread creation error. id:" + err);
  }
